...
export default function MarketplacePage() {
        ...
        fetch(`${config.api}/marketplace/listings`, {
            ...
            if (Array.isArray(JSON.parse(localStorage.getItem("CartItems")))) {
                ...
                const sortedReturnOther = sort(toReturnOther).desc(
                    (item) =>
                        (Number(item.item.item.item_value) / 1000) * item.rate
                );
    ...
    const handleMarketplaceUpdate = useCallback(
        (data) => {
            ...
            if (Array.isArray(JSON.parse(localStorage.getItem("CartItems")))) {
                ...
                setRenderOtherListings(
                    sortedOther.slice(0, observationIndexDebounced)
                );
                setOtherListings(sortedOther);
            }
            const sorted = sort(loadedItems).desc(
    ...
    const handleListingsRefresh = useCallback(() => {
        ...
        if (Array.isArray(JSON.parse(localStorage.getItem("CartItems")))) {
            ...
            if (existingCartItems) {
                ...
                setRenderOtherListings(
                    toReturn.slice(0, observationIndexDebounced)
                );
    ...
    const handleCartAdd = useCallback(
        (listing) => {
            if (userData) {
                if (
                    Array.isArray(JSON.parse(localStorage.getItem("CartItems")))
                ) {
                    const existingCartItems = JSON.parse(
                        localStorage.getItem("CartItems")
                    );
                ...
                <div className="Filter">
                    ...
                    <div className="Info">
                        <div className="Copyright">
                            <h3>
                                © {new Date().getFullYear()} GrowPvP.COM All
                                Rights Reserved
                            </h3>
                            <p className="SubText">
                                GrowPvP.COM is not affiliated, associated, or
                                partnered with Growtopia Corporation in any way.
                                We are not authorized, endorsed, or sponsored by
                                Growtopia Corporation.
                            </p>
                        </div>
                        <div className="Links">
                            <div
        ...
        <div className="MyListing" key={Information._id}>
            <div className="Information">
                ...
                <div className="ItemInfo">
                    <div className="Info">
                        <p className="Name">
                            {Information.item.item.display_name}
                        </p>
                        <div className="PriceInfo">
                            <p className="Price">
                                $
                                {numeral(
                                    (Number(Information.item.item.item_value) /
                                        1000) *
                                        Information.rate
                                ).format("0.00")}
                            </p>
                ...
                <div className="EditInfo">
                    ...
                    <p className="Rate">
                        ...
                        <span className="RateSpan">
                            <img src={redLightning} alt="lightning icon" />$
                            {dynamicRate}/1k
                        </span>
                    </p>
                </div>
